...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Cancel 25 queries, buffer-unordered by 3");
    cancel_queries_buffer_unordered(5, 3).await?;
    Ok(())
}

async fn cancel_queries_buffer_unordered(
    n: usize,
    buf_factor: usize,
    ...
    let receive = spawn(async move {
        receive_task_buffer_unordered_cancelling(rx, buf_factor, &valid_reader, &counter_writer)
            .await;
    });
...
async fn receive_task_buffer_unordered_cancelling(
